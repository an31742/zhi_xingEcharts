...
export default {
  ...
  mounted () {
    ...
    this.gradeChart = this.Echarts.init(this.$refs.grade, null, { renderer: 'svg' })
  ...
  methods: {
    ...
    toQuery () {
      sessionStorage.setItem('lifecycleDatasParams', JSON.stringify(this.lifecycleDatasParams))
      sessionStorage.setItem('queryName', '产品')
    ...
    prevFn () {
      const res = BoardServiceApi.toShowOfIndex('prev', 3, this.life3DShow)
      if (res && res.option) {
    ...
    nextFn () {
      const res = BoardServiceApi.toShowOfIndex('next', 3, this.life3DShow)
      if (res && res.option) {
        this.life3DChart.setOption(res.option)
        this.stageLst = res.stageLst
      }
    ...
    getLifecycleDatas () {
      BoardServiceApi.getLifecycleDatas(this.lifecycleDatasParams, this.life3DShow).then(res => {
        this.life3DChart.setOption(res.option)
        this.stageLst = res.stageLst
      })
    },
