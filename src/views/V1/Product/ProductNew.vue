    ...
    <div class="content">
      ...
      <div class="notice_content">
        <marquee  behavior=scroll direction=left align=middle>
          <span><b>事业群：</b>{{info.groupName || ''}}</span>
          <span><b>部门：</b>{{info.departmentName || ''}}</span>
          <span><b>产品介绍：</b>{{info.detail || ''}}</span>
        </marquee>
      </div>
...
import BoardServiceApi from '@/service/V1/board-service-new'
  ...
  methods: {
    ...
    getMonthIncomeAndUserOne () {
      ...
      BoardServiceApi.getMonthIncomeAndUserOne(params).then(res => {
        ...
        let userOption = res[1]
        userOption.yAxis[0].axisLabel.formatter = function (v, i) {
          return v / 10000
        }
        userOption.yAxis[1].axisLabel.formatter = function (v, i) {
          return v / 10000
        }
    ...
    getUserTotal () {
      ...
      BoardServiceApi.getUserTotal(params).then(res => {
        let openOption = res.open
        openOption.legend.top = '50%'
        let activeOption = res.active
        activeOption.legend.top = '70%'
        let payOption = res.pay
        payOption.grid.right = '5%'
        payOption.xAxis.axisLine.lineStyle = { color: '#29C896' }
        this.openChart.setOption(openOption)
        this.payChart.setOption(payOption)
